...
document.addEventListener("DOMContentLoaded", () => {
    var country = ("India")
	var city = ("Bhopal")
    var check = document.querySelector("#checkbx");
   
    if (!country || !city || !check) {
        console.error("One or more elements not found in the DOM");
        return;
    }

    check.addEventListener("click", () => {
        let key = `42f222cbcbca16e49a1a28d717db3c73`;
        let url = `https://api.openweathermap.org/data/2.5/weather?q=${city.value},${country.value}&lang=en&units=metric&appid=${key}`;

        console.log(`Fetching weather data for: ${city.value}, ${country.value}`);
        fetch(url)
            .then(response => {
            ...
            .then(data => {
                ...
                if (data.cod === 200) { // Ensure the response is successful
                    ...
                    alert('City not found. Please check the input.');
                }
