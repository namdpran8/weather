...
init();									// ⚙ initialize app
$(window).resize(onResize); 			// watch for window resize
requestAnimationFrame(tick);			// start animations

function init()
{
	onResize();	
	for(let i = 0; i < weather.length; i++)   // 🖱 bind weather menu buttons
	{
		let w = weather[i];
		let b = $('#button-' + w.type);
		w.button = b;
	...
	for(let i = 0; i < clouds.length; i++)			 // ☁️ draw clouds
